...
#打开open office这个软件
# openoffice4
echo " begin Current time: $(date '+%Y-%m-%d %H:%M:%S')"
sleep 1
#进入ppt界面
for i in {1..2}; do
	xdotool key Down
...
sleep 2
echo "title_edit begin Current time: $(date '+%Y-%m-%d %H:%M:%S')"
sleep 1
#插入修改标题
...
sleep 2
#增加插入图片的功能
echo "picture_edit  Current time: $(date '+%Y-%m-%d %H:%M:%S')"
sleep 1
...
sleep 1
# 增加文本框，加入文本
echo "text_edit  Current time: $(date '+%Y-%m-%d %H:%M:%S')"
sleep 1
...
echo "demonstrate Current time: $(date '+%Y-%m-%d %H:%M:%S')"
#放映
sleep 1
xdotool key F5
sleep 1
...
echo "rename Current time: $(date '+%Y-%m-%d %H:%M:%S')"
sleep 1
#rename
...
xdotool key ctrl+shift+s
#删除
echo "delete Current time: $(date '+%Y-%m-%d %H:%M:%S')"
sleep 1
